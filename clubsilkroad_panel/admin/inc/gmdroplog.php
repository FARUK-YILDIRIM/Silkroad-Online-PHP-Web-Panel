...
<header><h3 class="tabs_involved">[GM] DROP LOG</h3></header>
		...
		<?php 
			...
			$admin = new Admin();
			$degerler =$admin->link->db_conn_account->query("select * from _SMCLog where szLog like '%DROP%' ORDER BY dLogDate DESC");
			...
			if(isset($_POST['durum1'])){
				...
				$degerler =$admin->link->db_conn_account->query("select * from _SMCLog where szUserID ='$username' AND szLog like '%DROP%' ORDER BY dLogDate DESC ");
			...
			$degerler =$admin->link->db_conn_account->query("DELETE FROM _SMCLog WHERE szLog like '%DROP%'");
