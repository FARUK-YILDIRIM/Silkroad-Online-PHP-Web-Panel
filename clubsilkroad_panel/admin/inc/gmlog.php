...
<header><h3 class="tabs_involved">[GM] LOGİN LOG</h3></header>
		...
		<?php 
			...
			$admin = new Admin();
			$degerler =$admin->link->db_conn_log->query("SELECT * FROM fy_gm_log ORDER BY tarih DESC");
			...
			if(isset($_POST['durum1'])){
				...
				$degerler =$admin->link->db_conn_log->query("SELECT * FROM fy_gm_log WHERE username = '$username' ORDER BY tarih DESC");
			...
			if(isset($_POST['durumip'])){
				...
				$degerler =$admin->link->db_conn_log->query("SELECT * FROM fy_gm_log WHERE ip = '$ipsearch' ORDER BY tarih DESC");
				if($degerler->rowCount() == 0){
			...
			$degerler =$admin->link->db_conn_log->query("DELETE FROM fy_gm_log");
